# Đã dùng 86% bộ nhớ … Bạn có thể giải phóng bộ nhớ hoặc mua thêm bộ nhớ cho Drive, Gmail và Google Photos.
...
class SPEA2:
    ...
    def init_pop(self):
        self.pop = []
        self.pop = [Solution(np.random.uniform(MIN_VAL, MAX_VAL, NUM_VAR))for i in range(self.population_size)]
        self.archive = []
